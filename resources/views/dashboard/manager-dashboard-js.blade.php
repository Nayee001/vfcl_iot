                ...
                $.ajax({
                    ...
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr(
                            'content') // Ensures CSRF protection
                    },
                    cache: false,
                    processData: false,
                    contentType: false,
                    success: function(resp) {
                        if (resp.code ===
                            200
                        ) { // Use numeric comparison instead of `{{ __('statuscode.CODE200') }}`
        ...
        function loadDevices() {
            $.ajax({
                ...
                success: function(devices) {
                    ...
                    if (devices.length > 0) {
                        devices.forEach(device => {
                            let statusBadge = device.device_assigned.connection_status ===
                                'Authorized' ?
                            ...
                            if (device.device_assigned.status === 'Not Responded' || device
                                .device_assigned.status === 'Reject') {
                                pendingDevices.push(device.name);
                            }
                        ...
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-primary btn-sm" onclick="showDeviceLogs(${device.id})">View Logs</button>
                                <button class="btn btn-info btn-sm" onclick="viewWaveChart(${device.id})">Wave Chart</button>
                                <button class="btn btn-warning btn-sm" onclick="showActivateDeviceModal(${device.id})">Authenticate</button>
                            </td>
                        </tr>
                    `;
                        });
                        ...
                        <div class="alert alert-warning">
                            <strong>Pending Authorization:</strong> ${pendingDevices.join(', ')}
                        </div>
                    `;
                        }
                    } else {
                        deviceTable =
                            `<tr><td colspan="5" class="text-center">No devices assigned yet.</td></tr>`;
                    }

                    $('#deviceTableBody').html(deviceTable);
                    $('#top-message').html(topMessageHtml);
                },
                error: function() {
                    $('#deviceTableBody').html(
                        '<tr><td colspan="5" class="text-center text-danger">Failed to load devices.</td></tr>'
                        );
                }
            });
        }
